...
def bias_add_forward(x, b):
    ...
    """
    # add b to each row of x and cache b's shape for the backward pass
    y = x + b
    return {
        'y': y,
...
def bias_add_backward_db(dy, cache):
    ...
    # sum the upstream gradient over the batch dimension to get db of shape (D,)
...
def relu_backward(dy, cache):
    ...
    # return dx with gradient zeroed where the cached input was non-positive.
    x = cache['x']
    return dy * (x > 0)
...
def softmax_cross_entropy_backward(probs, targets):
    ...
    # produce the (B, V) gradient of mean cross-entropy w.r.t. logits.
...
def layernorm_forward_mean(x):
    ...
    # compute the per-row mean of x, preserving the reduced axis as size 1
...
def layernorm_forward_variance(x, mean):
    ...
    """
    # compute per-row variance using mean and return a (B, 1) array
...
def layernorm_forward_normalize(x, mean, var, eps):
    ...
    # subtract the per-row mean and divide by sqrt(var + eps)
...
def layernorm_forward_affine(x, gamma, beta, eps):
    ...
    # normalize each row to zero mean / unit variance, then apply gamma and beta.
...
def layernorm_backward_subtract_mean(dy, cache):
    ...
    """
    # compute the gradient contribution of the subtract-mean op
    x, mean = cache['x'], cache['mean']
...
def create_token_embedding(vocab_size, d_model, scale=0.02):
    ...
    # return a (vocab_size, d_model) array of small random values controlled by scale
...
def token_embedding_forward(token_ids, embedding_matrix):
    ...
    """
    # look up the embedding row for each token id and build the cache
